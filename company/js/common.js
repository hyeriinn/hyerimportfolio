...
let mainSlide = new Swiper('.main_slide_wrap', {
    ...
    on: {
        // autoplayTimeLeft 콜백: time은 남은 시간(ms), progress는 남은 비율(1 ~ 0) - autoplay가 있어야 적용됨
...
function updatePagination(swiper) {
    const currentNum = swiper.realIndex + 1;//현재 슬라이드 번호(0부터 시작이라 + 1)
    const formattedNum = currentNum < 10 ? `0${currentNum}` : currentNum; //10 미만 숫자앞에 0붙여주기
    document.querySelector('.swiper-pagination-current').textContent = formattedNum;
}
...
document.addEventListener('mousemove', (e) => {
    ...
    // const scrollY = 
    cursor.style.left = `${e.clientX}px`;//가로 위치
    cursor.style.top = `${e.clientY + scrollY}px`; //세로 위치
})
...
visionCard.forEach((card, i) => {
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: '.vision',
            start: `top+=${i * 300} center`, //300px 움직일때마다 ,띄어쓰기 x
...
let buisinessSwiper = new Swiper('.business .slide', {
    loop: true,
    slidesPerView: 1.2,
    spaceBetween: 30,
...
gsap.timeline({
    scrollTrigger: {
        trigger: '.vid_box',
        start: 'top 10%',
        end: "top 10%+=1500", //1500px 스크롤동아 애니메이션
        scrub: 1,
        pin: '.vid_box', //고정
        pinSpacing: true,
...
}).fromTo(cards, { y: 300, z: 200, opacity: 0.8 }, { y: 0, z: 0, duration: 1, ease: "power2.out", stagger: 1, opacity: 1 }).to(cards,{y:-300,z:-200,skewY:5,duration:1,ease: "power2.in", stagger: 1,},'+=0.5') //stragger : 카드 하나씩 1초 간격으로 차례차례 등장
//'+=0.5' : 앞 애니메이션 끝나고 0.5초 쉬었다가 등증
